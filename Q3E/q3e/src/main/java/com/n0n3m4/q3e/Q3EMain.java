...
//        datadir = PreferenceManager.getDefaultSharedPreferences(this).getString(Q3EUtils.pref_datapath, Q3EUtils.q3ei.default_path);
        datadir = this.getApplicationContext().getSharedPreferences("Q4A_Pref", 0).getString(Q3EUtils.pref_datapath, Q3EUtils.q3ei.default_path);
        Log.d("DataPath", "Your Data Dir Is: " + datadir);
//      if ((datadir.length() > 0) && (datadir.charAt(0) != '/'))//lolwtfisuserdoing?
        if ((datadir.isEmpty())) {
            datadir = "/" + datadir;
//            PreferenceManager.getDefaultSharedPreferences(this).edit().putString(Q3EUtils.pref_datapath, datadir).apply(); //.commit()
            this.getApplicationContext().getSharedPreferences("Q4A_Pref", 0).edit().putString(Q3EUtils.pref_datapath, datadir).apply(); //.commit()
        }
        if (checkGameFiles()) {
            if (mGLSurfaceView == null) {
                ShowMessage("SurfaceView Null, Initializing");
                mGLSurfaceView = new Q3EView(this);
            }
            ShowMessage("Set Content");
            setContentView(mGLSurfaceView);
            ShowMessage("Request Focus");
            mGLSurfaceView.requestFocus();
            if (mAudio == null) {
                ShowMessage("Init Audio");
                mAudio = new Q3ECallbackObj();
                mAudio.vw = mGLSurfaceView;
            }
            ShowMessage("CallBack Audio");
