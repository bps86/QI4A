...
//        int msaa = PreferenceManager.getDefaultSharedPreferences(this.getContext()).getInt(Q3EUtils.pref_msaa, 0);
        int msaa = this.getContext().getSharedPreferences("Q4A_Pref", 0).getInt(Q3EUtils.pref_msaa, 0);
        switch (msaa) {
            case 0:
                msaa = 0;
                break;
...
//        if (PreferenceManager.getDefaultSharedPreferences(this.getContext()).getBoolean(Q3EUtils.pref_32bit, false)) {
        if (this.getContext().getSharedPreferences("Q4A_Pref", 0).getBoolean(Q3EUtils.pref_32bit, false)) {
    ...
    public static final String detectev17 = "EV=17";
    ...
    public static String detectmouse() {
        try {
            Command command = new Command(0, "cat /proc/bus/input/devices") {
                @Override
                public void commandOutput(int id, String line) //public void output(int id, String line)
                {
                    if (line == null) return;
...
//		RootTools.getShell(true).add(command).waitForFinish();
    ...
    public Thread readmouse = new Thread(new Runnable() {
        ...
        public void run() {
            try {
                Command getenforce = new Command(0, "getenforce") {
                    @Override
                    public void commandOutput(int id, String line) //public void output(int id, String line)
                    {
                        if (line == null) return;
                        SELinuxEnforced = line.contentEquals("Enforcing");
                    }


                };
//			RootTools.getShell(true).add(getenforce).waitForFinish();
                RootTools.getShell(true).add(getenforce);

                if (SELinuxEnforced) {
//				CommandCapture command_setenforce0 = new CommandCapture(0, "setenforce 0");
                    Command command_setenforce0 = new Command(0, "setenforce 0");
//				RootTools.getShell(true).add(command_setenforce0).waitForFinish();
                    RootTools.getShell(true).add(command_setenforce0);
                }
...
//			RootTools.getShell(true).add(command).waitForFinish();
                ...
                if (SELinuxEnforced) {
//				CommandCapture command_setenforce1 = new CommandCapture(0, "setenforce 1");
                    Command command_setenforce1 = new Command(0, "setenforce 1");
//				RootTools.getShell(true).add(command_setenforce1).waitForFinish();
                    RootTools.getShell(true).add(command_setenforce1);
                }
...
//            SharedPreferences mPrefs = PreferenceManager.getDefaultSharedPreferences(this.getContext());
            SharedPreferences mPrefs = this.getContext().getSharedPreferences("Q4A_Pref", 0);
...
//        if ((Build.VERSION.SDK_INT >= 9) && (hideonscr) && (event.getSource() == InputDevice.SOURCE_MOUSE)) {
        if ( (hideonscr) && (event.getSource() == InputDevice.SOURCE_MOUSE)) {
