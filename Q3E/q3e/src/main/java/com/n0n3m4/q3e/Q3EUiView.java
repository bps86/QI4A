...
public class Q3EUiView extends GLSurfaceView implements GLSurfaceView.Renderer {
    ...
    class MenuOverlay extends Paintable implements TouchListener {
        ...
        public void onTouchEvent(int x, int y, int act) {
            Log.d("Q3E_UIview", "TouchEvent" );
            Log.d("Q3E_UIview", "act: "+act );
            if (act == 1) {
                mover_down = true;
                final Runnable rn;
                if (y < cy) {
    ...
    public void UiOnTouchEvent(FingerUi fn, MotionEvent event) {
        Log.d("Q3E_UIview", "PointerAction: "+event.getActionMasked() );
        int act = 0;
        ...
        Log.d("Q3E_UIview", "Act result: "+ act );
...
//		Editor mEdtr=PreferenceManager.getDefaultSharedPreferences(getContext()).edit();
        Editor mEdtr = getContext().getSharedPreferences("Q4A_Pref", 0).edit();
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if ((event.getActionMasked() == MotionEvent.ACTION_DOWN) || (event.getActionMasked() == MotionEvent.ACTION_POINTER_DOWN)) {
            ...
            if (mover.isInside(x, y)) {
                ...
                Log.d("Q3E_UIview", "Mover Inside" );
            } else {
                for (TouchListener tl : touch_elements) {
                    if (tl.isInside(x, y)) {
                        fingers[pid].target = tl;
                        Log.d("Q3E_UIview", "Target tl, break at " +touch_elements.indexOf(tl));
                        break;
                    }
                    Log.d("Q3E_UIview", "Continue Target");
                }
            }
        ...
        Log.d("Q3E_UIview", "Fingers: "+fingers.length );
        for (FingerUi f : fingers)
            if (f.target != null) {
                if (f.target != mover && !touch_elements.contains((TouchListener) f.target)){
                    f.target = null;
                    Log.d("Q3E_UIview", "Fingers nullify at "+f.id );
                } else {
